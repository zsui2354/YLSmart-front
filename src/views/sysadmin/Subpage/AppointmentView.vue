...
export default {
  ...
  methods: {
    ...
    async handleCreateAppointment() {
      try {
        // 格式化时间
        let time = this.createForm.appointment_time
        if (time instanceof Date) {
          const pad = n => n < 10 ? '0' + n : n
          time = `${time.getFullYear()}-${pad(time.getMonth()+1)}-${pad(time.getDate())} ${pad(time.getHours())}:${pad(time.getMinutes())}:${pad(time.getSeconds())}`
        }
        // 构造请求体
        const payload = {
          user_id: Number(this.createForm.user_id),
          service_id: Number(this.createForm.service_id),
          appointment_time: time,
          status: Number(this.createForm.status),
          note: this.createForm.note
        }
        await axios.post('http://localhost:8089/api/user/service/submitAppointment', payload, {
          headers: { 'Content-Type': 'application/json' }
        })
        ElMessage.success('预约创建成功')
        this.createDialogVisible = false
      } catch (error) {
        console.error('新建预约失败:', error)
        ElMessage.error('新建预约失败')
      }
    },
    async handleDeleteAppointment(row) {
      try {
        await this.$confirm('确认删除该预约吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
        await axios.post('http://localhost:8089/api/sysadmin/service/cancelAppointment', null, {
          params: { id: row.id }
        })
